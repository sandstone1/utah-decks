...
// import in the GoogleMap, useJsApiLoader and Marker hooks from the package
// " @react-google-maps/api "
import { GoogleMap, useJsApiLoader, Marker } from '@react-google-maps/api';
// import in the Spinner component
import Spinner from '../spinner/spinner-bounce-dark';
// import in the scss file
import styles from './maps-javascript-api.module.scss';
...
export default function MapsJavascriptAPIComponent() {
    ...
    // #1
    // for Google Maps we need to get a loader variable using the useLoadScript that we
    // imported in from above ( please see referenced video for details )
    const { isLoaded } = useJsApiLoader(
        {
            // get our api key and save it to the const " googleMapsApiKey "
    ...
    return (
        ...
        <Fragment>
            ...
            }
            {
                ...
                // remember, we had to use " isLoaded " and not " isLoading "
                !isLoaded ? (
                    ...
                    <GoogleMap
                        // there 3 props we need to pass to GoogleMap in order for
                        // everything to work properly and those are zoom, center
                        // and MapContainerClassName
                        zoom={ 10 }
                        // use the site :
                        // " https://www.latlong.net/convert-address-to-lat-long.html "
                        // to convert a street address into lat and lng coordinates
                        center={ { lat : 40.433620, lng : -111.857770 } }
                        // remember, the css for this class is found in the global.scss
                        // file, not in the css file for this component ( remember,
                        // we're doing things according the Google API )
                        // mapContainerClassName='mapContainer'
                        mapContainerStyle={ { maxWidth : '144.0rem', height : '80.0rem', margin : '0 auto' } }
                        // mapId={ 'mymapid' }
                        options={ 
                            {
                    ...
                    >
                        <Marker
                            position={ { lat : 40.433620, lng : -111.857770 } }
                        />
                    </GoogleMap>
